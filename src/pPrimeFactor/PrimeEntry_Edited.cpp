...
bool   PrimeEntry::factor(unsigned long int max_steps)
{
 ...
 //divide m_orig by the list of factors, to get current state
    if(m_factors.size()>0)
    {  
      for(int p2=0;p2!=m_factors.size();p2++) {
      m_current_value = m_current_value/m_factors[p2];
      }

	//      int length_m_factors = m_factors.size();
      // int counter = 0;
      // while(counter < length_m_factors)
      //	{
      //
      // counter = counter +1;
      //	}
...
string  PrimeEntry::getReport()
{
  ...
  time_t solve_time = 0;//m_stop_time - m_start_time; 
  stringstream sst;
  sst << solve_time;
