...
bool PrimeFactor::Iterate()
{
  ...
  int count =0;
  list<PrimeEntry>::iterator p_prime_entry;
  for(p_prime_entry=m_prime_entries.begin();p_prime_entry!=m_prime_entries.end(); )
    {
      //instance of prime entry
      ...
      //if the entry is done
      if(entry.done())
	{
	  entry.setCalculatedIndex(m_calc_index); 
	  string result_str = entry.getReport();
	  p_prime_entry = m_prime_entries.erase(p_prime_entry);  
	  Notify("RESULT", result_str);   
