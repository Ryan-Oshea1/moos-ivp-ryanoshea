...
bool HazardMgr::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    else if(key == "UP_LOITER")
      handleMailSharedSummary(sval);
...
{
  if( m_host_community =="kasper")
    {
      //  Notify("UHZ_CONFIG_REQUEST", "vname=kasper, width=10, pd=0.70");
    }
...
bool HazardMgr::handleMailDetectionReport(string str)
{
  ...
  //send event out to the other vehicle
  Notify("NODE_MESSAGE_LOCAL", "src_node=kasper,dest_node=jake,var_name=UP_LOITER,string_val=\"x=" + doubleToString(new_hazard.getX(),1) + ",y=" + doubleToString(new_hazard.getY(),1) + ",label=" + hazlabel + "\"");
...
void HazardMgr::handleMailReportRequest()
{
  ...
  //create a substring from the total string if it is longer than 100 characters 
  if( summary_report.length() > 100)
    {
      //      summary_report = "greater than 100";
    }
  //else
    {  
      if( m_host_community == "jake")
    {
      //      Notify("NODE_MESSAGE_LOCAL", "src_node=jake,dest_node=kasper,var_name=UP_LOITER,string_val=\""  +summary_report + "\"");
    }
  else
    { 
      //    Notify("NODE_MESSAGE_LOCAL", "src_node=kasper,dest_node=jake,var_name=UP_LOITER,string_val=\"" + summary_report + "\"");
    }
    }
  Notify("HAZARDSET_REPORT", summary_report + m_summary_report_shared);
...
void HazardMgr::handleMailSharedSummary(string str)
{
  //concatenate the new mail onto the shared summary variable so that we can add all of the data to the summary report 
  

  m_summary_report_shared =  m_summary_report_shared + "#" + str;
  Notify("COMM_HOST", str);

  //vectorize the summary report
  vector<string> f_summary_report_shared_vector = parseString(m_summary_report_shared,'#');
  
  for(unsigned int i=2; i<f_summary_report_shared_vector.size(); i++)
    {
      Notify("S_S_1", f_summary_report_shared_vector[i]);

      //then add it to a string that will get concatenated onto the summary report sent to shore
      m_summary_report_shared_parsed = m_summary_report_shared_parsed + "#" + f_summary_report_shared_vector[i];
      Notify("S_S_1", m_summary_report_shared_parsed);
    }
